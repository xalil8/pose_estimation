...
def run(poseweights="yolov7-w6-pose.pt",source="football1.mp4",device='cpu',view_img=False,
        save_conf=False,line_thickness = 15,hide_labels=False, hide_conf=True):
                ...
                for i, pose in enumerate(output_data):  # detections per image
                    ...
                    if len(output_data):  #check if no pose
                        ...
                        for det_index, (*xyxy, conf, cls) in enumerate(reversed(pose[:,:6])): #loop over poses for drawing on frame
                            ...
                            if frame_count>120 and int(xyxy[0])<1800:   #red
                                plot_one_box_kpt(xyxy, im0, label=label, color=colors(c, True), 
                                                xalil= (0,0,255),line_thickness=opt.line_thickness,kpt_label=True, kpts=kpts, steps=3, 
                                                orig_shape=im0.shape[:2])


                            else:            #blue
                                plot_one_box_kpt(xyxy, im0, label=label, color=colors(c, True), 
                                            xalil= (255,0,0),line_thickness=opt.line_thickness,kpt_label=True, kpts=kpts, steps=3, 
                                            orig_shape=im0.shape[:2])
